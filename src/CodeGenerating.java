...
public class CodeGenerating extends Visitor {
	...
	static int labelCnt = 0; // counter used to generate unique labels
	...
	void branchZNot(String label) {
		// Generate branch to label if stack top contains 1:
		// ifne label
		gen("ifne", label);
	}
	...
	void computeAdr(nameNode name) {
		...
		// dont load the value addressed onto the stack
		if (name.subscriptVal.isNull()) {
			// Simple (unsubscripted) identifier
			if (name.varName.idinfo.kind == ASTNode.Kinds.Var || name.varName.idinfo.kind == ASTNode.Kinds.ScalarParm) {
	...
	void storeId(identNode id) {
		if (id.idinfo.kind == ASTNode.Kinds.Var || id.idinfo.kind == ASTNode.Kinds.Value) {
			// id is a scalar variable
			if (id.idinfo.adr == ASTNode.AdrMode.global) // ident is a global
	...
	static Boolean isConditionalOp(int op) {
		switch (op) {
		case sym.CAND:
		case sym.COR:
			return true;
	...
	static String relationCode(int tokenCode) {
		switch (tokenCode) {
		case sym.EQ:
			return "eq";
		case sym.NOTEQ:
	...
	void genConditionalOp(binaryOpNode n) {
		//handle short-circuiting for CAND
		String trueLab = genLab();
		String falseLab = genLab();
		String skip = genLab();
		visit(n.leftOperand);
		if (n.operatorCode == sym.CAND){
			branchZ(falseLab);
		} else {
			branchZNot(trueLab);
		}
		visit(n.rightOperand);
		branchZ(falseLab);
		defineLab(trueLab);
		loadI(1);
		branch(skip);
	...
	boolean isNumericLit(exprNodeOption e) {
		return (e instanceof intLitNode) || (e instanceof charLitNode) || (e instanceof trueNode)
	...
	String buildTypeCode(exprNode n) {
		if (n.kind == ASTNode.Kinds.ArrayParm || n.kind == ASTNode.Kinds.Array)
	...
	void visit(asgNode n) {
		...
		if (n.source.kind == ASTNode.Kinds.Array || n.source.kind == ASTNode.Kinds.ArrayParm)
	...
	void visit(binaryOpNode n) {
		if (isConditionalOp(n.operatorCode)) {
			genConditionalOp(n);
			return;
		}
	...
	void visit(nameNode n) {
		if (n.subscriptVal.isNull()) {
			// Simple (unsubscripted) identifier
			if (n.varName.idinfo.kind == ASTNode.Kinds.Var || n.varName.idinfo.kind == ASTNode.Kinds.ScalarParm
	...
	void visit(methodDeclNode n) {
		...
		// well guestimate stack depth needed at 25
	...
	void visit(arrayDeclNode n) {
		...
		} else {// Process local array declaration
			// Give this variable an index equal to numberOfLocals
			// and remember index in symbol table entry
		...
		loadI(n.arraySize.intValue); // Push number of array elements
	...
	void visit(unaryOpNode n) {
		...
		if (n.operand instanceof charLitNode) {
			loadI(127);
			gen("ixor");
		} else if (n.operand instanceof intLitNode || n.operand instanceof bitStringNode) {
			loadI(-1);
			gen("ixor");
		} else if (n.operand instanceof trueNode) {
			loadI(0);
			gen("iand");
		} else {
			loadI(1);
